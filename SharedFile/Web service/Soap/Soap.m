...
+ (CXMLNode*) getNode: (CXMLNode*) element withName: (NSString*) name {
	for(CXMLNode* child in [element children]) {
		if([child respondsToSelector:@selector(localName)] && [[child performSelector:@selector(localName)] isEqual:name]) {
...
+ (id) deserialize: (CXMLNode*) element{
	...
	if(type == nil || type.length == 0) {
		...
		if([type isEqualToString:@"long"] || [type isEqualToString:@"unsignedlong"]) {
			return [NSNumber numberWithLong:(long)[value longLongValue]];
		}
...
//#warning This convertion needs to be looked at in detail as @"YES" could be @"true" or @"1"!!!
		return [NSNumber numberWithBool:(([[value lowercaseString] isEqualToString: @"YES"]) ? YES : NO)];
	}
...
+ (NSDateFormatter*)dateFormatter {
	...
	if(formatter == nil) {
		...
		[formatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss.SSS"];//yyyy-MM-dd HH:mm:ss
	}
	return formatter;
}
