...
@implementation BTFilterController{
    ...
    // To keep track of the current state, if there are changes, this will reflect it
    NSArray *_oldSelectionArray;
    NSArray *_oldItemArray;
}
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    // If the view dissappear but the topview is still showing this,
    // The Filter is actually embedded in iPad, so it actually has been "pop"
    // OR The the parents has been removed, that means this is really popped from Nav stack
    if (self.navigationController.topViewController == self || self.parentViewController == nil) {
        if (_delegate && [_delegate respondsToSelector:@selector(filterController:didFinishSelectionArray:)]) {
            [_delegate filterController:self didFinishSelectionArray:_selectionArray];
        }
        
        if (_delegate && [_delegate respondsToSelector:@selector(filterController:didFinishSelectionArray:hasChangedSelection:)]) {
...
- (NSMutableArray *)verifySelectionArray:(NSMutableArray *)selectionArray
{
    ...
    _oldItemArray = [[NSMutableArray alloc] initWithArray:_itemArray copyItems:YES];
...
{
    if ([_oldSelectionArray isEqual:_selectionArray] && [_oldItemArray isEqual:_itemArray]) {
        return YES;
    } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BTCellType cellType = [rowDict[KEY_2_CELL_TYPE] intValue];
    ...
    if (cellType == BTCellTypeCustomCell) {
        ...
        if (!cell) {
            ...
            DLog(@"CHECK delegate for BTFilterViewController or filterController:cellForIndexPath:label:height:");
        }
        return cell;
    }
    ...
    if (!cell.isSetup) {
        [cell setupWithInputType:cellType delegate:self];
   ...
   // NSLog(@"%@selectValueselectValueselectValue",selectValue);
      //NSLog(@"%@selectValueselectValueselectValue",_selectionArray);
    ...
    switch (cellType) {
        ...
        case BTCellTypeInputChoice:
            ...
            break;
        case BTCellTypeInputText:
                        if ([selectValue isKindOfClass:[NSNumber class]]) {
                 NSString* daysString = [[NSString alloc] initWithFormat:@"%@", selectValue];
  [cell.cellTextFeild setText:daysString];
                 
                  ///NSLog(@"%@BTCellTypeInputTextBTCellTypeInputTextBTCellTypeInputText",daysString);
             
             }
             else{
                 [cell.cellTextFeild setText:selectValue];
                 //NSLog(@"%@BTCellTypeInputTextBTCellTypeInputTextBTCellTypeInputText",selectValue);
        ...
        case BTCellTypeDeleteAction:
            [cell.cellButton setTitle:rowDict[KEY_2_LABEL] forState:UIControlStateNormal];
            break;
        default:
            break;
    }
...
{
    [self.tableView endEditing:YES];
    switch ([_itemArray[indexPath.section][KEY_1_ROW_ARRAY][indexPath.row][KEY_2_CELL_TYPE] intValue]) {
        case BTCellTypeInputBool:
            //do nothing
            break;
        case BTCellTypeInputChoice:{
        ...
        case BTCellTypeInputSlider:{
            ...
            if ([cell sliderToggle:cell.cellSlider.alpha]) {
                ...
                if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
                    [_delegate filterController:self valueForIndexPath:indexPath rowDict:_itemArray[indexPath.section][KEY_1_ROW_ARRAY][indexPath.row] changedTo:@(cell.cellSlider.value)];
                }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    } else if ([rowDict[KEY_2_CELL_TYPE] intValue] == BTCellTypeDisplayText) {
...
- (void)filterCell:(BTFilterCell *)filterCell switchChangedTo:(BOOL)boolValue
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
        [_delegate filterController:self valueForIndexPath:filterCell.indexPath rowDict:_itemArray[filterCell.indexPath.section][KEY_1_ROW_ARRAY][filterCell.indexPath.row] changedTo:@(boolValue)];
    }
}
...
- (void)filterCell:(BTFilterCell *)filterCell textChangedTo:(NSString *)textValue
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
        [_delegate filterController:self valueForIndexPath:filterCell.indexPath rowDict:_itemArray[filterCell.indexPath.section][KEY_1_ROW_ARRAY][filterCell.indexPath.row] changedTo:textValue];
    }
}
...
- (void)filterCell:(BTFilterCell *)filterCell attTextChangedTo:(NSAttributedString *)attTextValue
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
        [_delegate filterController:self valueForIndexPath:filterCell.indexPath rowDict:_itemArray[filterCell.indexPath.section][KEY_1_ROW_ARRAY][filterCell.indexPath.row] changedTo:attTextValue];
    }
}
...
- (void)filterCell:(BTFilterCell *)filterCell sliderValueChangedTo:(CGFloat)floatValue
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
        [_delegate filterController:self valueForIndexPath:filterCell.indexPath rowDict:_itemArray[filterCell.indexPath.section][KEY_1_ROW_ARRAY][filterCell.indexPath.row] changedTo:@(floatValue)];
    }
}
...
- (void)subFilterController:(BTSubFilterController *)subFilterController didChooseDict:(NSDictionary *)itemDict
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(filterController:valueForIndexPath:rowDict:changedTo:)]) {
        [_delegate filterController:self valueForIndexPath:subFilterController.indexPath rowDict:_itemArray[subFilterController.indexPath.section][KEY_1_ROW_ARRAY][subFilterController.indexPath.row] changedTo:itemDict];
    }
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(cellType)} mutableCopy];
    if (defaultValue) {
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeInputChoice), KEY_2X_CHOICE_ARRAY:choiceArray} mutableCopy];
    if (lastChoice) {
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeInputText), KEY_2X_AUTO_CORRECT: @YES} mutableCopy];
    if (defaultString) {
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeInputText)} mutableCopy];
    if (defaultString) {
        dict[KEY_2X_DEFAULT] = defaultString;
    }
    if (placeHolder) {
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeInputTextBox), KEY_2X_HEIGHT:@(height)} mutableCopy];
...
+ (NSDictionary *)rowInputBoolWithLabel:(NSString *)label defaultBoolean:(BOOL)defaultBoolean
{
    return  @{KEY_2_LABEL: label,
              KEY_2_CELL_TYPE: @(BTCellTypeInputBool),
...
{
    return  @{KEY_2_LABEL: label,
              KEY_2_CELL_TYPE: @(BTCellTypeInputSlider),
              KEY_2X_MIN:@(min),
              KEY_2X_MAX:@(max),
              KEY_2X_STEP:@(step)};
}
...
{
    return  @{KEY_2_LABEL: label,
              KEY_2_CELL_TYPE: @(BTCellTypeInputSliderRange),
              KEY_2X_MIN:@(min),
              KEY_2X_MAX:@(max),
              KEY_2X_STEP:@(step),
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeAction)} mutableCopy];
    if (defaultValue) {
        dict[KEY_2X_DEFAULT] = defaultValue;
    }
    return dict;
...
{
    NSMutableDictionary *dict = [@{KEY_2_LABEL: label, KEY_2_CELL_TYPE: @(BTCellTypeDeleteAction)} mutableCopy];
    if (defaultValue) {
        dict[KEY_2X_DEFAULT] = defaultValue;
    }
    return dict;
...
{
    NSMutableDictionary *dict = [@{KEY_2_CELL_TYPE: @(BTCellTypeDisplayImage), KEY_2X_HEIGHT: @(height)} mutableCopy];
    if (image) {
...
{
    return  @{KEY_2_CELL_TYPE: @(BTCellTypeDisplayText),
...
{
    return  @{KEY_2_CELL_TYPE: @(BTCellTypeDisplayKeyValue),
              KEY_2_LABEL: key,
...
{
    NSMutableDictionary *dict = [@{KEY_2_CELL_TYPE: @(BTCellTypeCustomCell), KEY_2X_HEIGHT: @(height)} mutableCopy];
    if (label) {
...
{
    NSMutableDictionary *dict = [@{KEY_2_CELL_TYPE: @(BTCellTypeCustomView), KEY_2X_HEIGHT: @(height)} mutableCopy];
    if (label) {
        dict[KEY_2_LABEL] = label;
    }
    return dict;
